...
class BaseLangChainSandboxTool(BaseTool, Generic[BaseSandboxSettingsT]):
    """
    A subclass of LangChain's 'BaseTool' that can interact with Agent Sandbox.

    Attributes:
        SANDBOX_ADAPTER_CLS: Class of the adapter that has to hadnle actual
        execution of a sandbox.
    ...
    # Override the following fields to make them non-mandatory,
    # since we set them from an adapter
...
class LangChainSandboxTool(BaseLangChainSandboxTool):
    """
    Base LanChain sandbox class that uses normal sandbox client.
    """
