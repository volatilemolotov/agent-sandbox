...
- [Composing Sandbox with Network Policies](/docs/examples/network-policies/) — Shows how to compose `Sandbox` with `NetworkPolicy`, `Ingress`, and `Service` resources using [KRO (Kubernetes Resource Orchestrator)](https://kro.run/docs/overview). Defines a higher-level `AgenticSandbox` CRD via a `ResourceGraphDefinition` that bundles these resources together. Also covers custom controller and Helm approaches.
- [Manual PodDisruptionBudget Configuration](/docs/examples/manual-pdb/) — Demonstrates a shared PDB per namespace approach using `maxUnavailable: 0` to protect all labeled sandbox pods from voluntary eviction. Sandboxes opt in via the `sandbox-disruption-policy: "manual-protection"` label. Note: manual PDBs do not clean themselves up — coordinate with your team before deletion.
